...
class predict_dataset(Dataset):
    def __init__(
        ...
        ok_event_id=None,
        ok_events_index=None,
        p_picks=None,
        mask_waveform_sec=3,
        sampling_rate=200,
        data_length_sec=30,
        max_station_number=25,
    ...
    def __getitem__(self, index):
        ...
        with h5py.File("D:/TEAM_TSMIP/data/TSMIP.hdf5", "r") as f:
            ...
            for i, eventID in enumerate(specific_index):  # trace loop
                ...
                if i == 0:
                    first_p_pick = p_pick = f["data"][str(eventID[0])]["p_picks"][
                        eventID[1]
                    ]
            ...
            if (
                len(stations_location) < self.max_station_number
            ):  # triggered station < max_station_number (default:25) zero padding
                for zero_pad_num in range(
                    self.max_station_number - len(stations_location)
                ):
            ...
            if self.mask_waveform_sec:
                Specific_waveforms[
                    :, first_p_pick + (self.mask_waveform_sec * self.sampling_rate) :, :
                ] = 0
...
for i in [4]:
    ...
    predict_data = predict_dataset(
        predict_catalog=data.event_metadata,
