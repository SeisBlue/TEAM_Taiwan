...
for model_index, model_name in enumerate(["Vp model", "Vs model"]):
    ...
    for dep_index in range(0, len(dep_coor)):
        ...
        cp = ax.contourf(
            X,
            Y,
            reshape_array[model_index, dep_index, :, :],
        ...
        fig.savefig(
            f"model_image/{model_name}_depth_{int(dep_coor[dep_index]*1000)}m.png",
            dpi=300,
        )
...
with open("vel3d_new.mod", "w") as file:
    points_info = lines[0]
    updated_points_info = points_info.replace("76", f"{76+len(increased_lon)}").replace(
        "61", f"{61+len(increased_lat)}"
    )
