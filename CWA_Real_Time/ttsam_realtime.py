...
def earthworm_wave_listener():
    while True:
        ...
        try:
            ...
            wave_buffer[wave_id] = np.roll(wave_buffer[wave_id], -wave["data"].size)
            wave_buffer[wave_id][-wave["data"].size :] = wave["data"]

            time_buffer[wave_id] = np.roll(time_buffer[wave_id], -wave["data"].size)
        ...
        try:
            ...
            for pick_id, pick in picks.items():
                ...
                for i, component in enumerate(["Z", "N", "E"]):
                    wave_id = (
                        f"{network}.{station}.{location}.{channel[0:2]}{component}"
                    )
                    print(wave_id)
